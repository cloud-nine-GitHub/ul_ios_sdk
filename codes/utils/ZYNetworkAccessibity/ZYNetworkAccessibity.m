...
static NSString *appName = @"defaultApp";
...
- (void)setupNetworkAccessibity {
    ...
    //_becomeActiveCallbacks = [[NSMutableArray array] retain];
    _becomeActiveCallbacks = [NSMutableArray new];
    
    BOOL firstRun = ({
        //TODO 如果没有点击网络选择重启机器，那么此逻辑就要修改
        NSString * appRunFlag = [appName stringByAppendingString:@"FirstRunFlag"];
        BOOL value = [[NSUserDefaults standardUserDefaults] boolForKey:appRunFlag];
        if (!value) {
            [[NSUserDefaults standardUserDefaults] setBool:YES forKey:appRunFlag];
        }
        !value;
    });
...
- (void)cleanNetworkAccessibity {
    ...
    //[_becomeActiveCallbacks release];
