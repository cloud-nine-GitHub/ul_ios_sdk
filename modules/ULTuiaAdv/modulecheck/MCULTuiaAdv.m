...
@interface MCULTuiaAdv ()<UITextFieldDelegate>
...
- (void)addListener
{
    [[ULNotificationDispatcher getInstance] addNotificationWithObserver:self withName:UL_NOTIFICATION_MC_SHOW_TUIA_ADV_CALLBACK withSelector:@selector(advCallback:) withPriority:-1];
}
...
//    }
    //未输入参数则使用本地默认的参数
    NSMutableDictionary *advData = [self getModuleAdvTestDataWithModule:@"ULTuiaAdv" withType:_advType withEditParam:_advParam withLocalParamKey:key];
...
- (void)openAdv:(NSMutableDictionary *)advData
{
    ...
    if([type isEqualToString:UL_ADV_URL]){
        ...
        NSArray *array = @[[NSString stringWithFormat:@"%d",ULA_GAME_ADV_INFO],@"ULTuiaAdv",UL_ADV_FULLSCREEN,@"branchAdvRequest",@"",@"",S_CONST_ADV_MC_ADVID_DES,S_CONST_ADV_MC_ADVID_DES,@"",@""];
        [[ULNotificationDispatcher getInstance] postNotificationWithName:UL_NOTIFICATION_ACCOUNT_UP_DATA withData:array];
        
        if (![[ULNotificationDispatcher getInstance] postNotificationWithName:UL_NOTIFICATION_MC_SHOW_TUIA_URL_ADV withData:advData]) {
