...
- (void)addListener
{
    [[ULNotificationDispatcher getInstance] addNotificationWithObserver:self withName:UL_NOTIFICATION_MC_SHOW_HUILIANG_ADV_CALLBACK withSelector:@selector(advCallback:) withPriority:-1];
}
...
- (void)showLabelClick
{
    ...
    NSString *key = @"";
    if([_advType isEqualToString:UL_ADV_FULLSCREEN]){
        key = @"s_sdk_adv_huiliang_fullscreenid";
    }else if([_advType isEqualToString:UL_ADV_VIDEO]){
        key = @"s_sdk_adv_huiliang_videoid";
    }else if([_advType isEqualToString:UL_ADV_INTERSTITIAL]){
        key = @"s_sdk_adv_huiliang_interid";
    }
    //未输入参数则使用本地默认的参数
    NSMutableDictionary *advData = [self getModuleAdvTestDataWithModule:@"ULHuiliangAdv" withType:_advType withEditParam:_advParam withLocalParamKey:key];
...
- (void)openAdv:(NSMutableDictionary *)advData
{
    ...
    if ([type isEqualToString:UL_ADV_SPLAH]) {
        ...
        NSArray *array = @[[NSString stringWithFormat:@"%d",ULA_GAME_ADV_INFO],@"ULHuiliangAdv",UL_ADV_FULLSCREEN,@"branchAdvRequest",@"",@"",S_CONST_ADV_MC_ADVID_DES,S_CONST_ADV_MC_ADVID_DES,@"",@""];
        [[ULNotificationDispatcher getInstance] postNotificationWithName:UL_NOTIFICATION_ACCOUNT_UP_DATA withData:array];
        
        if (![[ULNotificationDispatcher getInstance] postNotificationWithName:UL_NOTIFICATION_MC_SHOW_HUILIANG_FULLSCREEN_ADV withData:advData]) {
            [MCULModuleLayoutCreater showTipsWithTitile:@"提示" withDesc:@"广告消息未注册,请检查cop是否配置该广告" withBtnText:@"知道了"];
        }
    }else if([type isEqualToString:UL_ADV_VIDEO]){
        
        //广告请求统计 对于多参数来说并不知道本次请求的是哪一个参数
        NSArray *array = @[[NSString stringWithFormat:@"%d",ULA_GAME_ADV_INFO],@"ULHuiliangAdv",UL_ADV_VIDEO,@"branchAdvRequest",@"",@"",S_CONST_ADV_MC_ADVID_DES,S_CONST_ADV_MC_ADVID_DES,@"",@""];
        [[ULNotificationDispatcher getInstance] postNotificationWithName:UL_NOTIFICATION_ACCOUNT_UP_DATA withData:array];
        
        if (![[ULNotificationDispatcher getInstance] postNotificationWithName:UL_NOTIFICATION_MC_SHOW_HUILIANG_VIDEO_ADV withData:advData]) {
            [MCULModuleLayoutCreater showTipsWithTitile:@"提示" withDesc:@"广告消息未注册,请检查cop是否配置该广告" withBtnText:@"知道了"];
        }
        ...
        if (![[ULNotificationDispatcher getInstance] postNotificationWithName:UL_NOTIFICATION_MC_SHOW_HUILIANG_INTER_ADV withData:advData]) {
            [MCULModuleLayoutCreater showTipsWithTitile:@"提示" withDesc:@"广告消息未注册,请检查cop是否配置该广告" withBtnText:@"知道了"];
        }
    }
}
