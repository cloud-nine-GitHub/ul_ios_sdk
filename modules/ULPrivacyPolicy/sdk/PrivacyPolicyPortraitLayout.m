...
@implementation PrivacyPolicyPortraitLayout

static PrivacyPolicyPortraitLayout *instance = nil;

+ (id) getInstance{
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        if (instance == nil) {
            instance = [[PrivacyPolicyPortraitLayout alloc] init];
        }
    });
    return instance;
...
- (UIView *)getPrivacyPolicyParentView
{
    ...
    constraintTop = [NSLayoutConstraint constraintWithItem:relative attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:parentView attribute:NSLayoutAttributeTop multiplier:1.0 constant:100];
    constraintLeft = [NSLayoutConstraint constraintWithItem:relative  attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:parentView attribute:NSLayoutAttributeLeft multiplier:1.0 constant:30];
    constraintRight = [NSLayoutConstraint constraintWithItem:parentView attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:relative attribute:NSLayoutAttributeRight multiplier:1.0 constant:30];
    constraintBottom = [NSLayoutConstraint constraintWithItem:parentView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:relative attribute:NSLayoutAttributeBottom multiplier:1.0 constant:100];
    ...
    constraintTwoTop = [NSLayoutConstraint constraintWithItem:relativeTwo attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:relative attribute:NSLayoutAttributeTop multiplier:1.0 constant:30];
    constraintTwoLeft = [NSLayoutConstraint constraintWithItem:relativeTwo  attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:relative attribute:NSLayoutAttributeLeft multiplier:1.0 constant:20];
    constraintTwoRight = [NSLayoutConstraint constraintWithItem:relative attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:relativeTwo attribute:NSLayoutAttributeRight multiplier:1.0 constant:20];
    constraintTwoBottom = [NSLayoutConstraint constraintWithItem:relative attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:relativeTwo attribute:NSLayoutAttributeBottom multiplier:1.0 constant:25];
    ...
    /*webview*/
    WKWebView *webView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 0, [ULTools getCurrentViewController].view.bounds.size.width-100, [ULTools getCurrentViewController].view.bounds.size.height-335) configuration:[self getConfiguration]];
    ...
    /*底部按钮相对布局*/
    UIView *relativeBtn = [[UIView alloc] initWithFrame:CGRectMake(0, [ULTools getCurrentViewController].view.bounds.size.height-335, [ULTools getCurrentViewController].view.bounds.size.width-100, 80)];
    [relativeTwo addSubview:relativeBtn];
    
    /*webview与布局按钮的分界线*/
    UIView *divideLine = [[UIView alloc] initWithFrame:CGRectMake(0, 5, [ULTools getCurrentViewController].view.bounds.size.width-100, 1)];
    divideLine.backgroundColor = [UIColor grayColor];
    [relativeBtn addSubview:divideLine];
    
    
    /*同意按钮背景图片**/
    UIImageView *agreeImgView=[[UIImageView alloc] initWithFrame:CGRectMake(0, 20, [ULTools getCurrentViewController].view.bounds.size.width-100, 36)];
    ...
    agree.frame = CGRectMake(0, 22, [ULTools getCurrentViewController].view.bounds.size.width-100, 32);
    agree.text = @"同意，继续游戏";
    ...
    refuse.frame = CGRectMake(0,65, refuseWidth,refuseHeight);
