...
@interface MCULGdtAdv ()<UITextFieldDelegate>
...
- (void)addListener
{
    [[ULNotificationDispatcher getInstance] addNotificationWithObserver:self withName:UL_NOTIFICATION_MC_SHOW_GDT_ADV_CALLBACK withSelector:@selector(advCallback:) withPriority:-1];
}
...
- (void)showLabelClick
{
    ...
    //未输入参数则使用本地默认的参数
    NSMutableDictionary *advData = [self getModuleAdvTestDataWithModule:@"ULGdtAdv" withType:_advType withEditParam:_advParam withLocalParamKey:key];
...
- (void)openAdv:(NSMutableDictionary *)advData
{
    ...
    if ([type isEqualToString:UL_ADV_SPLAH]) {
        ...
        NSArray *array = @[[NSString stringWithFormat:@"%d",ULA_GAME_ADV_INFO],@"ULGdtAdv",UL_ADV_FULLSCREEN,@"branchAdvRequest",@"",@"",S_CONST_ADV_MC_ADVID_DES,S_CONST_ADV_MC_ADVID_DES,@"",@""];
        [[ULNotificationDispatcher getInstance] postNotificationWithName:UL_NOTIFICATION_ACCOUNT_UP_DATA withData:array];
        
        if (![[ULNotificationDispatcher getInstance] postNotificationWithName:UL_NOTIFICATION_MC_SHOW_GDT_INTER_ADV withData:advData]) {
            [MCULModuleLayoutCreater showTipsWithTitile:@"提示" withDesc:@"广告消息未注册,请检查cop是否配置该广告" withBtnText:@"知道了"];
        }
