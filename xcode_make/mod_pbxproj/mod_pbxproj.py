...
class XCBuildConfiguration(PBXType):
    ...
    # ul add begin
    def add_value(self, preprocessors, base, key, escape=True):
        modified = False
        ...
        for preprocessor in preprocessors:
            ...
            if preprocessor == '$(inherited)':
                escape = False

            if escape:
                if self[base][key].add('"%s"' % preprocessor):  # '\\"%s\\"' % preprocessor
                    modified = True
            else:
                if self[base][key].add(preprocessor):  # '\\"%s\\"' % preprocessor
                    modified = True
    ...
    def add_preprocessor(self, preprocessors):
        return self.add_value(preprocessors, 'buildSettings', 'GCC_PREPROCESSOR_DEFINITIONS')
    # ul add end
...
class XcodeProject(PBXDict):
    ...
    def add_preprocessor(self, preprocessors):
        build_configs = [b for b in self.objects.values() if b.get('isa') == 'XCBuildConfiguration']
        for b in build_configs:
            if b.add_preprocessor(preprocessors):
                self.modified = True
    ...
    def add_flags(self, pairs, configuration='All'):
        ...
        for b in build_configs:
            # print "  build_configs.b(%s)" % b
            if configuration != "All" and b.get('name') != configuration :
                print "  b.get('name')(%s)" % b.get('name')
    ...
    def add_embed_binaries(self, embed_binaries, naviTargetName):
        if len(embed_binaries) > 0:
            self.modified = True
            # 3d project need all,ReleaseForProfiling and ReleaseForRunning
            self.add_single_valued_flag("LD_RUNPATH_SEARCH_PATHS", "$(inherited) @executable_path/Frameworks")
            ...
            for binary in embed_binaries:
                self.add_embed_framework(binary, naviTargetName)
    def test(self):
        for f in self.objects.values():
            if f.get('isa') == 'PBXFileReference':
                print f
    def add_embed_framework(self,file_name, naviTargetName):
        file_refs = self.get_files_by_name(os.path.basename(file_name))
        if len(file_refs) > 0:
            file_ref = file_refs[0]
            embedPhase = self.add_embed_framework_build_phase(naviTargetName)
            if embedPhase == None:
    ...
    def add_embed_framework_build_phase(self, naviTargetName):
        phase = None
        naviTarget = targets = self.get_target_by_name(naviTargetName)#self.get_build_phases('PBXNativeTarget')[0]#
        if naviTarget == None :
